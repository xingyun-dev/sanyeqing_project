...
def send_email(receiver, ecode):
    # 邮箱账号和发件者签名
    sender = 'wht <2641938162@qq.com>'
    # 定义发送邮件的内容,支持HTML标签和CSS样式
    content = f"<br/>欢迎注册 三叶识青，您的邮箱验证码为：<span style='color:red;font-size:20px;'>{ecode}</span>，请在4分钟内填写完成注册，如非本人操作，请忽略此邮件。"
    # 实例化邮件对象,并指定邮件的关键信息。
    message = MIMEText(content, 'html', 'utf-8')
    # 指定邮件的标题,同样使用utf-8编码
    message['Subject'] = Header('三叶识青验证码', 'utf-8')
    ...
    smtpObj.login('2641938162@qq.com', 'uettpixszruvdjfj')
    # 指定发件人、收件人和邮件内容
...
def handle_upload_user(encoded_thumbnail):
    ...
    thumbname = time.strftime("%Y%m%d_%H%M%S." + "png")
    # 保存缩略图到文件
    image.save('./static/download/' + thumbname, format='png')
    compress_image('./static/download/' + thumbname, './static/chat_room/images/' + thumbname, 300)
...
# 下载图片
def handle_upload_photos(encoded_thumbnail):
    # 解码 base64 字符串
    data_url = encoded_thumbnail
    decoded_bytes = base64.b64decode(data_url)
    image_stream = io.BytesIO(decoded_bytes)
    ...
    # 保存缩略图到文件
    image.save('./static/download/' + thumbname, format='png')
    compress_image('./static/download/' + thumbname, './static/sanyeqing_uploads/' + thumbname, 300)
...
def ming_photos_by_type(type):
    primary_category = ""
    secondary_category = ""
    if type.startswith("1"):
        primary_category = "块根照片"
        if type == "11":
            secondary_category = "云南省"
        elif type == "12":
            secondary_category = "广西省"
        elif type == "13":
            secondary_category = "浙江省"
        elif type == "14":
            secondary_category = "贵州省"
        elif type == "15":
            secondary_category = "陕西省"
    elif type.startswith("2"):
        primary_category = "叶片照片"
        if type == "21":
            secondary_category = "云南省"
        elif type == "22":
            secondary_category = "广西省"
