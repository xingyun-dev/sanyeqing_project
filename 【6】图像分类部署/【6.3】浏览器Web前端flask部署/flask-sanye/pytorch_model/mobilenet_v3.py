...
# 定义最小卷积模块，这个写法比较少见，一般都是继承nn.Module，然后在forward()函数中定义模型，或者直接利用nn.Sequential()定义模型
class ConvBNActivation(nn.Sequential):  # 这种写法是直接利用nn.Sequential()定义模型的一种变种写法
    def __init__(self,
                 in_planes: int,
                 out_planes: int,
                 kernel_size: int = 3,
...
class MobileNetV3(nn.Module):
    def __init__(self,
        ...
        if norm_layer is None:
            norm_layer = partial(nn.BatchNorm2d, eps=0.001, momentum=momentum)
...
# 定义large结构
def mobilenet_v3_large(num_classes: int = 1000, width_multi: float = 1.0, reduced_tail: bool = False) -> MobileNetV3:
    """
