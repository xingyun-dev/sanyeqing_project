   ...
   export default {
     ...
     methods: {
         getComList() {
           ...
           uni.request({
             url: `https://www.whtuu.cn/api/images_my?userid=${this.userid}`,  // 修改接口地址并添加 userid 参数
             method: 'GET',
             success: (response) => {
               this.comList = response.data.map(image => {
                 return {
                   img: 'https://www.whtuu.cn/api/images/' + image.photos_name+'.png', // Assume that the response contains an array of objects with "photos_name" property
